...
public class HomeWork2 {
    ...
    /**
     * Задание:
     * 1. Создайте вложенный класс LoginValidationException, унаследуйте его от
     * Exception
     ...
     * - должен содержать как минимум одну маленькую, одну большую букву, цифру и
     * нижнее подчеркивание
     * - максимальная длинна логина- 20 символов
     * - если логин не соответствует требованиям - выбросить
    ...
    public static void validateLogin(String login) throws LoginValidationException {
        ...
        if (login.length() > 20) {
            ...
            for (int i = 0; i < login.length(); i++) {
                if (login.charAt(i) >= 48 && login.charAt(i) <= 57) {
                    nam = true;
                } else if ((login.charAt(i) >= 65 && login.charAt(i) <= 90)
                        || (login.charAt(i) >= 97 && login.charAt(i) <= 122)) {
                    simv = true;
                } else if (login.charAt(i) != 95) {
                    err = true;
                }
            }
            if (nam && simv && !err) {
    ...
    public static void printTestCase(int n, Boolean exp, Boolean act, int minLen) {
        ...
        System.out.print("Ожидание: " + yellow.apply(constLen(exp.toString(), minLen)) + " Реальность: "
                + green.apply(constLen(act.toString(), minLen) + " "));
