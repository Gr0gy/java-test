 ...
 * Если вас просят создать класс - его надо создать вложенным, т.е. внутри
 * класса HomeWorkN
 ...
 * ‼️ВАЖНО‼️ Вся работа в модкле ведется на JDK-17, перед запуском скачайте его
 * и установите
...
public class HomeWork3 {
     ...
     * Задание:
     *
     * 1. Реализуйте метод raiseException(ниже) который принимет число и выбрасывает
     * исключения:
     ...
     *
     * 2. Реализуйте метод catchException который вызывает raiseException затем
     * ловит каждое
    ...
    public static void raiseException(Integer exceptionId)
            throws SocketException, ClassNotFoundException, AccessDeniedException, ExportException {// Напиши свой код
                                                                                                    // тут для задания 1
        switch (exceptionId) {
            case 1:
            ...
            case 4:
                throw new ExportException("file");

        }
    }

    public static String catchException(Integer integer)
            throws SocketException, ClassNotFoundException, AccessDeniedException, ExportException {
    ...
    public static void testSecondTask() {
        ...
        for (Integer exceptionId : classesCorrelation.keySet().stream().sorted().toList())
            printTestCase(exceptionId, f.apply(classesCorrelation.get(exceptionId).toString()),
                    catchException(exceptionId), 30);
    }
    ...
    public static void printTestCase(int n, String exp, String act, int minLen) {
        ...
        System.out.print("Ожидание: " + yellow.apply(constLen(exp, minLen)) + " Реальность: "
